...
/**
 * Multi-column layout on an inner [#optireader-scroll] with [width: max-content] so columns extend
...
fun epubPaginatedCss(
    ...
    return """
        ...
        #optireader-scroll p, #optireader-scroll li, #optireader-scroll blockquote, #optireader-scroll pre, #optireader-scroll figure,
        #optireader-scroll h1, #optireader-scroll h2, #optireader-scroll h3, #optireader-scroll h4, #optireader-scroll h5, #optireader-scroll h6 {
        ...
        #optireader-scroll img, #optireader-scroll svg, #optireader-scroll video, #optireader-scroll canvas, #optireader-scroll table {
            break-inside: avoid !important;
            -webkit-column-break-inside: avoid !important;
            page-break-inside: avoid !important;
...
/** Paging uses transform on [#optireader-scroll]; metrics use its scrollWidth and Kotlin [contentWcss]. */
fun epubPageMetricsJs(contentWcss: Int): String = """
    (function() {
        var cw = Math.max(80, $contentWcss);
        var el = document.getElementById('optireader-scroll');
...
fun injectEpubPaginationCss(
    ...
    val js = """
        (function() {
            ...
            if (!body) return;
            var old = document.getElementById('optireader-scroll');
            if (old) {
                while (old.firstChild) body.insertBefore(old.firstChild, old);
                old.remove();
            }
            var oldViewport = document.getElementById('optireader-viewport');
            if (oldViewport) oldViewport.remove();
            var wrap = document.createElement('div');
            wrap.id = 'optireader-scroll';
            while (body.firstChild) wrap.appendChild(body.firstChild);
            var viewport = document.createElement('div');
            viewport.id = 'optireader-viewport';
            ...
            frame.id = 'optireader-frame';
            viewport.appendChild(frame);
            frame.appendChild(wrap);
            body.appendChild(viewport);
            var st = document.getElementById('optireader-pagination');
            if (!st) {
                st = document.createElement('style');
                st.id = 'optireader-pagination';
